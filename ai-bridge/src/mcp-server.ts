...
export class MCPServer {
  private readonly godot: GodotConnection;
  ...
  /** 连接到 Godot */
  private async _connectToGodot(): Promise<void> {
    try {
  ...
  /** 排空输入缓冲区 */
  private _drainBuffer(flush: boolean): void {
    while (true) {
  ...
  /** 处理单条消息 */
  private async _handleMessage(message: unknown): Promise<JsonRpcResponse | null> {
    ...
    // 通知消息（没有 id）— 不需要响应
    if (isJsonRpcNotification(msg as unknown as JsonRpcMessage)) {
      await this._handleNotification(msg.method as string, msg.params as Record<string, unknown>);
      return null;
    }

    // 请求消息 — 需要响应
    if (isJsonRpcRequest(msg as unknown as JsonRpcMessage)) {
  ...
  private async _handleRequest(
    ...
    switch (method) {
      ...
      case 'tools/call':
        return this._handleToolsCall(id, params as ToolCallParams | undefined);
  ...
  /** 处理 initialize 请求 */
  private _handleInitialize(id: string | number, params?: Record<string, unknown>): JsonRpcResponse {
  ...
  /** 处理 tools/list 请求 */
  private async _handleToolsList(id: string | number): Promise<JsonRpcResponse> {
    // 先尝试从缓存获取
    ...
    try {
      const result = await this.godot.request('tools/list') as { tools?: ToolDefinition[] };
  ...
  /** 格式化工具调用结果为 MCP 标准格式 */
  private _formatToolResult(result: unknown): ToolCallResult {
  ...
  private async _handleResourcesList(id: string | number): Promise<JsonRpcResponse> {
    // 尝试从 Godot 获取资源列表
    if (!this.godot.isConnected()) {
    ...
    try {
      const result = await this.godot.request('resources/list') as { resources?: ResourceDefinition[] };
  ...
  /** 处理 prompts/list 请求 */
  private async _handlePromptsList(id: string | number): Promise<JsonRpcResponse> {
    // 内置 prompts
  ...
  private _handlePromptsGet(
    ...
    switch (name) {
      case 'godot-development': {
        const task = (params?.arguments as Record<string, string>)?.task ?? '';
        const context = (params?.arguments as Record<string, string>)?.context ?? '';
        return this._buildResult(id, {
          description: 'Godot 游戏开发辅助提示',
          messages: [
            {
      ...
      case 'scene-analysis': {
        const scenePath = (params?.arguments as Record<string, string>)?.scene_path ?? '当前场景';
        return this._buildResult(id, {
          description: '场景结构分析提示',
          messages: [
            {
  ...
  /** 构建成功响应 */
  private _buildResult(id: string | number | null, result: unknown): JsonRpcResponse {
    return {
      jsonrpc: '2.0',
      id,
      result,
  ...
  /** 写消息到 stdout */
  private _writeMessage(message: JsonRpcResponse | JsonRpcResponse[]): void {
