...
export class BatchProcessor {
            ...
            merge: (commands) => {
                ...
                for (const cmd of commands) {
                    if (cmd.params.properties && typeof cmd.params.properties === 'object') {
                        Object.assign(properties, cmd.params.properties);
                    }
                }
                return {
                    method: 'create_node',
    ...
    /** 刷新并执行批处理命令（需要传入执行函数） */
    async flushAndExecute(executor) {
        // 取消定时器
        if (this.flushTimer !== null) {
            clearTimeout(this.flushTimer);
            this.flushTimer = null;
    ...
    _tryMerge(commands) {
        for (const rule of this.mergeRules) {
            ...
            if (key !== null) {
                // 检查所有命令是否属于同一组
                const allSameGroup = commands.every((cmd) => rule.groupKey(cmd.method, cmd.params) === key);
                if (allSameGroup) {
