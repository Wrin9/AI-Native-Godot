...
export class StateManager {
    ...
    /** 获取缓存的工具列表 */
    getCachedTools() {
        return this.get(StateManager.KEYS.TOOLS);
    }
    /** 设置缓存的工具列表 */
    ...
    applyEvent(event) {
        ...
        switch (event.type) {
            ...
            case 'node_property_changed': {
                // 节点属性变更，使场景树缓存失效
                this.invalidate(StateManager.KEYS.SCENE_TREE);
                this.deltaBuffer.modified[`node:${event.data.node}`] = event.data;
                this.deltaBuffer.timestamp = now;
                break;
            }
            case 'node_created':
            case 'node_deleted': {
                ...
                if (event.type === 'node_created') {
                    this.deltaBuffer.added[`node:${event.data.node}`] = event.data;
                }
                else {
                    this.deltaBuffer.removed.push(`node:${event.data.node}`);
                }
                this.deltaBuffer.timestamp = now;
                break;
            }
            ...
            case 'state_snapshot': {
                ...
                // 存储快照数据到各个缓存项
                if (event.data.tools) {
                    this.setCachedTools(event.data.tools);
                }
